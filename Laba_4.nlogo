globals [time  kol maxim rabtime l i j bol q ]
breed [komps komp]
breed [ps p]
...
to setup
    ...
    clear-all
reset-ticks
  create-komps kolvo_k [ set size 3 layout-circle sort turtles 10 ]
  ;; Выставляем черепашек в круг радиусом 12 и сразу сортируем их в порядке возрастания
  ask turtles [ set shape "circle"  set label who + 1 ] ;; Делаем черепашек в виде круга, задаем им цвет и свойтву label даем значение в виде порядкового номера

   create-ps kolvo_l [ set size 3 set shape "person" ]
...
set l random (kolvo_k - 1)
 ...
 if count ps < kolvo_l [
  ...
  if kol > 10 [create-ps 1 [set size 3 setxy -15 random-ycor set shape "person"
